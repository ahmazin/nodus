...
Nodus (`@nodus-dev/*`) is a headless, framework-agnostic diagram engine. Its security posture centers on
...
**In scope:** the published `@nodus-dev/*` packages (parsers, serialization, renderer, the MCP server, the
...
- **Parsers** (`@nodus-dev/from-mermaid`, `@nodus-dev/import-infra`, `@nodus-dev/text-to-diagram`) use bounded
