...
**Nodus** (`@nodus-dev/*`) is a headless, framework-agnostic, extensible **diagram engine** for TypeScript
...
as one preset (`@nodus-dev/preset-infra`) on top of the general core.

pnpm workspace monorepo. `@nodus-dev/core` has **zero framework/DOM dependencies**; `@nodus-dev/react` is a thin
...
`@nodus-dev/*` to `packages/*/src`. So the example app, tests, and the CLI all run **TypeScript source
...
- **Layouts** — `editor.registerLayout(engine)` then `await editor.layout('dagre', { direction: 'LR' })`.
  Adapters live in `@nodus-dev/layout-{dagre,tree,force,elk}`.
- **Plugins** — `editor.use(plugin)`; a plugin gets an `EngineHost` to register types/tools/themes/
  layouts/overlays and hook the store & event bus (`@nodus-dev/plugin-freehand` is a full example).
...
`@nodus-dev/core` has **deterministic/canonical serialization**; diagrams are stored as `*.nodus.json`. The
