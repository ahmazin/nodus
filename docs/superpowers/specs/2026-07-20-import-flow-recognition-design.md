...
- **Area:** `@nodus-dev/import-infra` (Terraform + Kubernetes recognition), `@nodus-dev/from-mermaid` (small non-breaking skip-count add), example app (`examples/browser/src/` — new `import-analyze.ts`, `import-editor.tsx`, `main.tsx`), `scripts/verify-import.mjs`.
...
- **A new `@nodus-dev/import` façade package.** Detection/orchestration lives in the example app for now; extract a package later if a second consumer (CLI/MCP) needs it.
...
2. **YAML:** add the `yaml` npm dependency to `@nodus-dev/import-infra`; `fromKubernetes` accepts `string | objects`. (Lockfile change — surfaced to the human at implementation time.)
...
### 2. Recognition — Terraform edges (`@nodus-dev/import-infra`)
...
### 3. Recognition — Kubernetes YAML + skip reporting (`@nodus-dev/import-infra`)
...
### 4. Recognition — Mermaid skip count (`@nodus-dev/from-mermaid`, minimal)
