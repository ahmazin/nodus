...
Take `@nodus/*` from a strong 0.1 engine to a **credible, shippable GA (1.0)**: a headless
...
- **Fix the P0 publish break:** `@nodus/react` imports `@nodus/icons-cloud` without declaring it, and
...
- **Autosave + open/save**: wire the unused `@nodus/persistence` package to the web layer —
...
- Tests for `layout-dagre` / `layout-tree` / `layout-force` (no-overlap/determinism, mirroring
  layout-elk).
- READMEs for `@nodus/core` and `@nodus/react`; root README polish.
...
- [ ] `@nodus/*` publish graph valid (no undeclared/private imports); `files`/`repository` set.
