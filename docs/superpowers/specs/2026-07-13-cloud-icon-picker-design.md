...
Let a user browse the curated cloud-service icons (`@nodus-dev/icons-cloud`, 92 AWS/Azure/GCP glyphs),
...
A new reusable component in `@nodus-dev/react`:
...
1. **`cloudIconCatalog`** (`@nodus-dev/icons-cloud`) — the data. Exported array derived from `ALLOWLIST`,
   the single source of truth. `@nodus-dev/react` never imports `@nodus-dev/icons-cloud`; the catalog arrives
   as a prop, keeping the component reusable for any icon set.

2. **canvas registry** (`@nodus-dev/react`, internal) — the DOM link. A module-level
...
3. **`CloudIconPicker`** (`@nodus-dev/react`) — the UI: button, popover, search/filter, preview grid,
...
`@nodus-dev/react` declares its **own** structurally-compatible `IconCatalogEntry` interface and takes
`catalog` as a prop, so it carries no build- or runtime dependency on `@nodus-dev/icons-cloud`. The
...
- `@nodus-dev/icons-cloud`: assert `cloudIconCatalog` has one entry per `ALLOWLIST` entry with matching
  `name/provider/service/category` (fold into the existing integrity test).
- `@nodus-dev/react`: unit-test `filterCatalog` (query substring, provider filter, empty query).
