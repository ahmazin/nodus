# Design: Schema & custom-shape migrations for `@nodus-dev/core`
...
- `Migration` type — exported from `@nodus-dev/core`.
...
typeVersions? }` — a breaking change to that exported function, accepted while `@nodus-dev/*` is
