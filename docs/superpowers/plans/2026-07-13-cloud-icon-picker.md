...
**Goal:** A reusable `<CloudIconPicker>` in `@nodus-dev/react` — a "Cloud" toolbar button opening a searchable popover whose scrollable grid of real icon previews can be dragged onto the canvas to place an icon node.

**Architecture:** `@nodus-dev/icons-cloud` exports a `cloudIconCatalog` derived from `ALLOWLIST`. `@nodus-dev/react` gains a JSX-free `cloud-icon-catalog.ts` (types + pure `filterCatalog`), a `canvas-registry.ts` (`WeakMap<Editor, HTMLCanvasElement>` the `<Nodus>` host fills on mount), and the `cloud-icon-picker.tsx` component. The component is decoupled from cloud icons (catalog is a prop). Drag placement maps a drop point to world coords via the registered canvas + `editor.screenToWorld`.
...
- `@nodus-dev/react` must NOT import `@nodus-dev/icons-cloud` (it declares its own structural `IconCatalogEntry` and takes `catalog` as a prop).
- Core `Editor` stays headless — no DOM references in `@nodus-dev/core`.
...
### Task 1: `cloudIconCatalog` in `@nodus-dev/icons-cloud`
...
### Task 2: Editor→canvas registry in `@nodus-dev/react`
...
**Interfaces:**
- Consumes: `Editor` type from `@nodus-dev/core`.
...
### Task 3: `IconCatalogEntry` + pure `filterCatalog` in `@nodus-dev/react`
 ...
 * filterCatalog directly. @nodus-dev/react declares its OWN IconCatalogEntry so it never depends on
 * @nodus-dev/icons-cloud — the cloudIconCatalog value is structurally assignable to it.
 */
export interface IconCatalogEntry {
  name: string;
  provider: 'aws' | 'azure' | 'gcp';
...
**Interfaces:**
- Consumes: `getIcon`, `type Ctx2D`, `type Editor` from `@nodus-dev/core`; `getCanvas` from `./canvas-registry.js`; `filterCatalog`, `IconCatalogEntry`, `ProviderFilter` from `./cloud-icon-catalog.js`.
...
**Interfaces:**
- Consumes: `CloudIconPicker` from `@nodus-dev/react`, `cloudIconCatalog` from `@nodus-dev/icons-cloud`.

- [ ] **Step 1: Replace the placeholder in the demo**

In `examples/browser/src/main.tsx`:

1. Add imports (next to the existing `@nodus-dev/react` and `@nodus-dev/icons-cloud` imports):

```ts
import { CommandPalette, CloudIconPicker, Minimap, Nodus, Properties, copyImage, useValue } from '@nodus-dev/react';
...
- `cloudIconCatalog` from `@nodus-dev/icons-cloud` derived from `ALLOWLIST`, taken as a prop → Task 1 (export) + Task 4 (prop). ✓
