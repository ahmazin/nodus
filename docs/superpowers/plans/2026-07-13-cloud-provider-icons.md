...
**Architecture:** A build-time codegen converts vendored official SVGs into compact vector "packs" (path-command data). A generic `drawVectorIcon` in core replays that data through the existing `Ctx2D` primitives — no `Ctx2D` change, no runtime assets, no async. A new `@nodus/icons-cloud` package holds the data and registers each icon under a `provider:service` name into core's icon registry, so `drawStencil`/`iconNode` draw them unchanged.
...
- Install converter dev deps: `pnpm --filter @nodus/icons-cloud add -D svgson svgpath` (run after Task 3 creates the package; listed here so it isn't forgotten).
...
### Task 4: `@nodus/icons-cloud` package + install/registration + fixture pack
...
**Interfaces:**
- Consumes: `registerIcon`, `drawVectorIcon`, `VectorIcon` from `@nodus/core`.
...
// packages/icons-cloud/package.json
{
  ...
  "dependencies": { "@nodus/core": "workspace:*" }
}
```
...
import type { VectorIcon } from '@nodus/core';
...
**Interfaces:**
- Consumes: `svgson` (`parseSync`), `svgpath`, `OP` + `VectorIcon`/`VectorSubpath` from `@nodus/core`.
- Produces: `function svgToVectorIcon(svg: string): { icon: VectorIcon; warnings: string[] }`.

- [ ] **Step 1: Add dev deps**

Run: `pnpm --filter @nodus/icons-cloud add -D svgson svgpath`
