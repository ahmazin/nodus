...
The contract covers exactly what a package exports from its barrel (`@nodus-dev/core`, `@nodus-dev/react`,
...
- **`@internal` symbols carry no stability promise.** Anything tagged `@internal` in its TSDoc — or
  reachable only through a deep import path (`@nodus-dev/core/src/...`) rather than the barrel — may
...
`@nodus-dev/core` serializes diagrams to **canonical** `*.nodus.json` — stable key order, normalized
