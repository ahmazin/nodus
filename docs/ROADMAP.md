...
**Axis 3 — layout**
- [x] Tree layout adapter (`@nodus/layout-tree`)
- [x] Force layout adapter (`@nodus/layout-force`)
- [x] ELK layout adapter (`@nodus/layout-elk`, worker-capable via workerUrl)
...
- [x] Freehand / sketch plugin (`@nodus/plugin-freehand`)
- [x] ERD visualizer (`@nodus/preset-diagrams`)
- [x] State-machine editor (`@nodus/preset-diagrams`)
...
- [x] Text → diagram (`@nodus/text-to-diagram`)
- [x] Live infra from Terraform / Kubernetes (`@nodus/import-infra`)
...
- [x] Persistence (`@nodus/persistence` + doc server; localStorage autosave in playground)
- [x] Draw preset (`@nodus/preset-draw`: rect/ellipse/diamond/text + line/arrow + R/E/L/A/T)
...
- [x] `fromMermaid` + ELK — `@nodus/from-mermaid` parses flowchart/state/ER subsets → reuses `@nodus/preset-diagrams` builders → ELK layout; `importMermaid(editor, src)`; verified headless render of all 3 subsets; live "⤓ Mermaid" import in the playground
- [x] MCP server (`@nodus/mcp`) — dependency-free stdio JSON-RPC around a headless `Editor`; 14 tools (import_mermaid, add/connect/update/delete, layout, **set_flow / set_flow_metric** for data-driven flow, export_png inline traffic-snapshot, export_json, save/load_doc); review-hardened (path containment, replace-on-import, graceful drain); verified via real stdio handshake
