...
`@nodus-dev/react` is a thin binding over `@nodus-dev/core`: a canvas host that wires pointer/wheel/keyboard to
...
`@nodus-dev/react` is a client component: the published build ships a `'use client'` banner, and its hooks
