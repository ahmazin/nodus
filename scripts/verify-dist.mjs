 ...
 * `main`/`types` at its `src` index and Vite/Vitest alias every `@nodus-dev` package to its source
...
// The @nodus-dev packages we pack + install into the consumer for BEHAVIORAL checks. icons-cloud is
// private (never publishes) but stays here to exercise its multi-entry build. cli and its @nodus-dev
...
// @nodus-dev package names that must never be referenced by a published manifest (unpublishable).
const PRIVATE_NODUS = new Set(
  readdirSync(join(ROOT, 'packages'))
    ...
    .map((name) => `@nodus-dev/${name}`),
);

// Packages whose manifests may keep @nodus-dev/core as a regular dependency: self-contained bin apps
// that PROVIDE the peer for the extension packages they bundle. Everything else must peer-depend.
const CORE_DEP_ALLOWED = new Set(['@nodus-dev/cli', '@nodus-dev/mcp']);
...
function main() {
  console.log('verify-dist — consuming built @nodus-dev/* packages from outside the workspace\n');
  ...
  mkdirSync(join(consumer, 'node_modules', '@nodus-dev'), { recursive: true });
  writeFileSync(
  ...
  for (const pkg of NODUS_PACKAGES) {
    ...
    // npm tarball naming: '@nodus-dev/core' + '0.1.0' -> 'nodus-core-0.1.0.tgz'
  ...
  for (const pkg of ALL_PUBLISHABLE) {
    ...
    try {
      ...
      record(`manifest @nodus-dev/${pkg}`, false, 'unreadable package.json in tarball');
      continue;
    }
    const issues = [];
    const warns = [];
    ...
    for (const field of ['dependencies', 'peerDependencies']) {
      for (const [dep, spec] of Object.entries(m[field] ?? {})) {
        if (!dep.startsWith('@nodus-dev/')) continue;
    ...
    if (m.dependencies?.['@nodus-dev/core'] && !CORE_DEP_ALLOWED.has(m.name)) {
      issues.push('@nodus-dev/core must be a peerDependency for extension packages');
    }
    ...
    record(
      `manifest @nodus-dev/${pkg}`,
      issues.length === 0,
      issues.join('; ') || (warns.length ? `WARN: ${warns.join('; ')}` : 'topology + completeness ok'),
    );
  }

  // 2) extract tarballs into the consumer's node_modules/@nodus-dev/*.
  ...
  for (const pkg of NODUS_PACKAGES) {
    ...
    if (rx.code !== 0 || !existsSync(extracted)) {
      record(`extract @nodus-dev/${pkg}`, false, rx.stderr.trim() || 'tar failed');
      finish();
      return;
    }
    // `tar` cannot cross filesystems on rename; use tar's own move via cp -R-free approach: rename.
    const dest = join(consumer, 'node_modules', '@nodus-dev', pkg);
    rmSync(dest, { recursive: true, force: true });
    execFileSync('mv', [extracted, dest]);
  }
  record('extract tarballs', true, `${NODUS_PACKAGES.length} packages into node_modules/@nodus-dev`);
  ...
  const resolveProbe = `
    ...
    const specs = ${JSON.stringify([...NODUS_PACKAGES.map((p) => `@nodus-dev/${p}`), ...Object.keys(EXTERNALS)])};
  ...
  record('dependency graph resolves', true, 'all @nodus-dev/* + externals require.resolve() cleanly');
  ...
  const esm = runConsumerScript(
    ...
    const esmUrl = import.meta.resolve('@nodus-dev/core');
    out['esm-resolves-dist'] = esmUrl.endsWith('/dist/index.js');
    const { Editor } = await import('@nodus-dev/core');
  ...
  const cjs = runConsumerScript(
    ...
    `
    const out = {};
    const entry = require.resolve('@nodus-dev/core');
    out['cjs-resolves-cjs'] = entry.endsWith('/dist/index.cjs');
    const { Editor } = require('@nodus-dev/core');
    const ed = new Editor();
  ...
  const dtsDir = join(consumer, 'node_modules', '@nodus-dev', 'core', 'dist');
  ...
  const subpaths = runConsumerScript(
    ...
    await probe('preset-infra-resolves', '@nodus-dev/preset-infra', (m) =>
      typeof m.InfraCanvas === 'function' && typeof m.installInfraPreset === 'function' && m.darkInfraTheme != null);
    await probe('layout-dagre-resolves', '@nodus-dev/layout-dagre', (m) => m.dagreLayout != null);
    await probe('react-entry-resolves', '@nodus-dev/react', (m) =>
  ...
  ingest(subpaths, {
    'preset-infra-resolves': '@nodus-dev/preset-infra resolves from dist (InfraCanvas etc.)',
    'layout-dagre-resolves': '@nodus-dev/layout-dagre resolves from dist (dagreLayout)',
    'react-entry-resolves': '@nodus-dev/react main entry resolves from dist (Nodus, useValue)',
  });
  ...
  // --- RSC boundary directive: @nodus-dev/react's shipped bundles must LEAD with 'use client' —
  ...
  for (const flavor of ['index.js', 'index.cjs']) {
    const f = join(consumer, 'node_modules', '@nodus-dev', 'react', 'dist', flavor);
  ...
  const binPath = join(consumer, 'node_modules', '@nodus-dev', 'cli', 'dist', 'bin.js');
  ...
  const mcpBin = join(consumer, 'node_modules', '@nodus-dev', 'mcp', 'dist', 'bin.js');
...
// node_modules/@nodus-dev/*). Bounded by a timeout; never hangs.
