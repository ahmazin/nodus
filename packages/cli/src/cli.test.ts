/**
 * @nodus-dev/cli — command behavior. Fixtures are written to a temp dir so the commands exercise real
 * file I/O (fmt writes in place; --check must not). Canonical bytes come from @nodus-dev/core so these
...
import { Editor, SCHEMA_VERSION, serializeRecords, toCanonicalString, type NodeRecord } from '@nodus-dev/core';
import { fromTerraform } from '@nodus-dev/import-infra';
...
describe('@nodus-dev/cli', () => {
  ...
  describe('fmt write-safety + exit codes (F41)', () => {
    ...
    it('refuses a file written by a newer Nodus with exit 3 and leaves it untouched', async () => {
      ...
      expect(errs.some((l) => l.includes('newer version of Nodus') && l.includes('upgrade @nodus-dev/cli'))).toBe(true);
    });
