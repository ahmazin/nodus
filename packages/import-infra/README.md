...
```bash
pnpm add @nodus-dev/import-infra @nodus-dev/core @nodus-dev/preset-infra
```
...
```ts
import { fromTerraform, fromKubernetes } from '@nodus-dev/import-infra';
...
- **Malformed input throws** an `ImportError` — a subclass of `NodusError` (from `@nodus-dev/core`) — with
...
import { analyzeKubernetes } from '@nodus-dev/import-infra';
