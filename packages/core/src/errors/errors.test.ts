...
describe('NodusError', () => {
  it('carries code, brand, name, message, and optional context/cause', () => {
    ...
    expect(e.brand).toBe('@nodus/core:NodusError');
...
describe('isNodusError (dual-package-safe brand check)', () => {
  ...
  it('matches a FOREIGN error with the same brand+code shape (the dual-package seam)', () => {
    // A NodusError minted by a SECOND copy of @nodus/core (an ESM build and a CJS build coexisting
    ...
    const foreign = { brand: '@nodus/core:NodusError', code: 'unknown-layout', message: 'from the other copy' };
  ...
  it('rejects a plain Error and non-branded / non-object values', () => {
    ...
    expect(isNodusError({ brand: '@nodus/core:NodusError' })).toBe(false); // no string `code`
