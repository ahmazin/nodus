 ...
 * other `@nodus/*` packages use a slash namespace (e.g. `'persistence/load-failed'`), so the brand
...
const NODUS_ERROR_BRAND = '@nodus/core:NodusError';
 ...
 *
 * **Never test membership with `instanceof`.** When `@nodus/core` is loaded twice in one process
...
/**
 * Structural type guard for a {@link NodusError} produced by ANY copy of `@nodus/core`. Matches on
